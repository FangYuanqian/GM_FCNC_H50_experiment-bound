...
// 计算每个 m_phi 区间内的 N_phi 的上下边缘，并输出到终端和 CSV
void calculate_N_phi_in_ranges(const std::vector<std::string>& csv_files, const std::string& output_file) {
    ...
    // 输出格式：m_phi Lower Bound, m_phi Upper Bound, N_phi Min, N_phi Max
    out_file << "m_phi Lower Bound,m_phi Upper Bound,N_phi Min,N_phi Max" << std::endl;

    for (const auto& csv_file : csv_files) {
        auto data = read_m_phi_theta_squared_from_csv(csv_file);

        double min_m_phi = std::numeric_limits<double>::max();
        double max_m_phi = std::numeric_limits<double>::lowest();

        for (const auto& [m_phi, _] : data) {
            min_m_phi = std::min(min_m_phi, m_phi);
            max_m_phi = std::max(max_m_phi, m_phi);
        }

        int num_points = 11; 
        double step = (max_m_phi - min_m_phi) / (num_points - 1);

        std::cout << "File: " << csv_file << " - N_phi range:" << std::endl;

        for (int i = 0; i < num_points - 1; ++i) {
            double m_phi_low = min_m_phi + i * step;
            double m_phi_high = min_m_phi + (i + 1) * step;
...
int main() {
    ...
    std::string output_file = "/home/fyq/DIYbyfyq/H5/data/167PS191output/167PS191NphiRangeByMass_output.csv";
