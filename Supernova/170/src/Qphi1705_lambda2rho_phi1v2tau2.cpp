...
//lambda_phi = Q_phi / rho_phi * MeV_to_1/cm
//v_phi_natural=Sqrt(1 - pow((m_phi / E), 2)); // 返回 v_phi，单位是自然单位制
...
double calculate_lambda_phi_inverse(double Q_phi, double rho_phi) {
    return Q_phi / rho_phi * MeV_to_inv_cm;
}

// 计算 lambda_phi
double calculate_lambda_phi(double lambda_phi_inverse) {
    if (lambda_phi_inverse <= 0) {
        std::cerr << "Invalid lambda_phi_inverse, must be > 0" << std::endl;
        return -1; // 表示错误
    }
...
// 计算能量 E = 3.16 * T_SN (MeV)
double calculate_energy() {
    return 3.16 * T_SN; // 返回 E，单位是 MeV
}

// 计算自然单位制下的速度 v_phi
double calculate_v_phi_natural(double m_phi, double E) {
    return sqrt(1 - pow((m_phi / E), 2)); // 返回 v_phi，单位是自然单位制
}

// 将速度转换为国际单位制，单位为 m/s
double convert_to_si(double v_phi_natural) {
    return v_phi_natural * c; // 转换为 cm/s
}

// 计算 Lorentz 因子 γ
double calculate_gamma(double v_phi_si) {
    double velocity_ratio = v_phi_si / c; // 速度与光速的比值
...
double calculate_escape_probability(double R_SN, double lambda_phi_inverse, double tau_phi, double sin_theta, double m_phi) {
    // 计算能量
    double E = calculate_energy(); // 能量 E (MeV)
    
    // 计算自然单位制下的速度
    double v_phi_natural = calculate_v_phi_natural(m_phi, E);
    
    // 将速度转换为国际单位制
    double v_phi_si = convert_to_si(v_phi_natural);
    ...
    // 调用 Lorentz 因子计算函数
    double local_gamma_phi = calculate_gamma(v_phi_si); // 传递正确的速度参数
...
int main() {
    ...
    // 打开 CSV 文件
    std::ofstream outputFile("/home/fyq/DIYbyfyq/Experimentbound/Supernova/170/data/data5.csv");
    if (!outputFile) {
...
// 生成多个随机点
 ...
 // 使用对数均匀分布生成 m_phi 和 sin_theta_squared
        ...
        // 计算 lambda_phi^{-1}
        double lambda_phi_inverse = calculate_lambda_phi_inverse(Q_phi, rho_phi);
        // 计算 lambda_phi
        double lambda_phi = calculate_lambda_phi(lambda_phi_inverse);
        // 计算总宽度和寿命
        ...
        // 计算 v_phi（自然单位制下的速度）
        double v_phi_natural = calculate_v_phi_natural(m_phi, E);
       // 将速度转换为国际单位制，单位为 cm/s
        double v_phi_si = v_phi_natural * c; // 转换为 cm/s
        double P_esc = calculate_escape_probability(R_SN, lambda_phi_inverse, tau_phi, sin_theta,v_phi_natural);
