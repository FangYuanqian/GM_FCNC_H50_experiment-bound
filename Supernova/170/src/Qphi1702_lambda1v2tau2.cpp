...
//lambda_phi = 2.15（F计算的完美还原sintheta范围的所需的系数） * 10.0 * 100.0(pow(F / 1e3, 2) /(1e12) )
// 计算能量 E = 3.16 * T_SN (MeV)  E越大，mphi才能越大
...
// 计算逃逸概率 P_esc
double calculate_escape_probability(double R_SN, double input_gamma_phi, double tau_phi, double sin_theta, double v_phi) {
    ...
    // 调用 Lorentz 因子计算函数
    double local_gamma_phi = calculate_gamma(v_phi);
...
// 计算粒子的寿命
double calculate_lifetime(double m_phi, double sin_theta, double hbar) {
    // 调用计算总衰变宽度的函数
    double Gamma = calculate_Gamma_total(m_phi, sin_theta);

    // 如果总宽度为 0，发出警告并返回 0
    if (Gamma == 0) {
        std::cerr << "Warning: 总宽度为 0,无法计算寿命。" << std::endl;
        return 0;
    }
...
int main() {
 ...
 // 使用对数均匀分布生成 m_phi 和 sin_theta_squared
       ...
       // 将速度转换为国际单位制，单位为 cm/s
        ...
        // 计算 gamma_phi
        double gamma_phi = calculate_gamma(v_phi_natural); // 这里需要传入 v_phi_natural

        // 计算逃逸概率 P_esc
        double P_esc = calculate_escape_probability(R_SN, gamma_phi, tau_phi, sin_theta, v_phi_natural); // 确保 v_phi_natural 被传入
