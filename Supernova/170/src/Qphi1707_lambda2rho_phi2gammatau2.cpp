...
// 被积函数
double integrand_energy_density(double E, double m_phi) {
...
double calculate_energy_density(double m_phi) {
    double integral_energy_density = 0.0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        double integrand_value = integrand_energy_density(E, m_phi);
        integral_energy_density += integrand_value;
    ...
    integral_energy_density *= h; // 积分结果乘以步长

    // 乘以前面的常数因子
    double final_result = (1 / (2 * PI * PI)) * integral_energy_density; // 使用 g 计算最终结果

    return final_result;
}

// 计算 lambda_phi^{-1}
double calculate_lambda_phi_inverse(double Q_phi, double rho_phi, double xi_value) {
...
double calculate_lambda_phi(double lambda_phi_inverse) {
    ...
    double lambda_phi = 1.0 / lambda_phi_inverse * 2.1894549080 * 2.15;
...
// 生成多个随机点
 ...
 // 使用对数均匀分布生成 m_phi 和 sin_theta_squared
        ...
        // 计算 rho_phi
        double rho_phi = calculate_energy_density(m_phi);
        // 计算 lambda_phi^{-1}
