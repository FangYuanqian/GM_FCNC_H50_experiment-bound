...
//(F >= 1e6 && F <= 6e11)(（F计算的完美还原sintheta范围的所需的系数）:6)
//lambda_phi = 2.15（（F计算的完美还原sintheta范围的所需的系数）） * 10.0 * 100.0(pow(F / 1e3, 2) /(1e12) ) cm
//vphi = lambda_phi / tau_phi
//tau_phi=1/（Gamma_e + Gamma_mu）;(tau似乎对图影响几乎没有)  
